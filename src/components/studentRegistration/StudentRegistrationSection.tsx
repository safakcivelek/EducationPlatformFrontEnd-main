...
const StudentRegistrationSection = () => {
    ...
    const registerService = new StudentRegisterService();
    const handleRegister = async (event: any) => {
        try {
            const response = await registerService.register({ firstName: event.firstName, lastName: event.lastName, email: event.email, password: event.password });
    ...
    const notify = (errorMessage: string) => {
        toast.error(`• ${errorMessage}`, {
            position: "top-right", autoClose: 2000, hideProgressBar: false, closeOnClick: true, pauseOnHover: true, draggable: true, progress: undefined, theme: "light",
        });
    };
    ...
    return (
        <>
                                                ...
                                                <div className="col-md-5">
                                                    <div className="signup-box text-center">
                                                        <div className="signup-text">
                                                            <h3>Öğrenci Üyelik Formu</h3>
                                                            <p style={{ marginBottom: "80px" } }>Global öğrenci topluluğumuza katılmak ister misiniz? Başvuru formunu doldurun. Ekibimiz sonraki adımları görüşmek üzere sizinle iletişime geçecektir.</p>
                                                        </div>
            ...
            <EmailVerificationModal show={showVerificationModal} onClose={() => setShowVerificationModal(false)} />
        </>
    );
};
